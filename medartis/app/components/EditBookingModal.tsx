            ...
            {options.length === 0 ? <div className="px-3 py-4 text-center text-xs font-mono opacity-40">No set options loaded.</div> : options.map((option) => {
              ...
              return (
                <label key={`${name}-${value}`} className={`mb-1 flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-2 text-xs transition ${checked ? setBadgeClasses(option) : 'border-base-200 hover:bg-base-50'}`}>
                  ...
                  <span className="flex-1"><span className="block font-black">{optionLabel(option)}</span><span className="text-[10px] opacity-60">{setStatusLabel(option)}</span></span>
                </label>
              );
            })}
          </div>
...
export default function EditBookingModal({ booking, isOpen, onClose, onSuccess, currentUserName, currentUserRole, salesPeople, hospitals }: EditBookingModalProps) {
  ...
  const requestedOptions = useMemo(() => {
    const byName = new Map<string, BookingSetOption>();
    availableSets.forEach((set) => byName.set(set.SetName || set.SetID, set));
    requestedSets.forEach((name) => { if (!byName.has(name)) byName.set(name, { SetID: name, SetName: name }); });
    return Array.from(byName.values()).sort((a, b) => (a.SetName || a.SetID).localeCompare(b.SetName || b.SetID));
  }, [availableSets, requestedSets]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-5xl overflow-hidden rounded-[30px] border border-base-300 bg-base-100 shadow-2xl">
        <form ref={formRef} onSubmit={handleSubmit}>
          <div className="border-b border-base-200 bg-gradient-to-r from-base-100 to-base-200/70 px-6 py-5"><div className="flex items-start justify-between gap-4"><div><div className="badge badge-primary badge-outline font-mono uppercase tracking-widest">Edit booking</div><h2 className="mt-3 text-xl font-black tracking-tight text-base-content">{booking.BookingID}</h2><p className="mt-1 text-sm text-base-content/70">{role === 'warehouse' ? 'Warehouse users can update only status and selected sets.' : 'Update booking details and operational notes.'}</p></div><button type="button" onClick={onClose} className="btn btn-ghost btn-sm rounded-full" aria-label="Close dialog">✕</button></div></div>
